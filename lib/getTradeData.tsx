...
export const tradeData = async () => {
  ...
  const fetchDayTrade = async (network: string, startTime: number, stringKey: string) => {
    ...
    if (network === optimism_url){
      const futuresVol = stringKey === "daily" ? (await getFutures()).kwentaDaily.yo.volume.toNumber() : 
      stringKey === "seven" ? (await getFutures()).kwentaSeven.yo.volume.toNumber() :
      stringKey === "thirty" ? (await getFutures()).kwentaThirty.yo.volume.toNumber() :
      stringKey === "ninety" ? (await getFutures()).kwentaNinety.yo.volume.toNumber() :
      0

      const futuresTrades = stringKey === "daily" ? (await getFutures()).kwentaDaily.yo.trades : 
      stringKey === "seven" ? (await getFutures()).kwentaSeven.yo.trades :
  ...
  // Daily
  const dailyTradeOvm = await fetchDayTrade(optimism_url, times.twentyFourHourAgo, "daily")
  ...
  const dailyTradeMain = await fetchDayTrade(mainnet_url, times.twentyFourHourAgo, "daily")
  ...
  // 7 day
  const sevenTradeOvm = await fetchDayTrade(optimism_url, times.sevenDayAgo, "seven")
  ...
  const sevenTradeMain = await fetchDayTrade(mainnet_url, times.sevenDayAgo, "seven")
  ...
  const thirtyTradeOvm = await fetchDayTrade(optimism_url, times.thirtyDayAgo, "thirty")
  ...
  const thirtyTradeMain = await fetchDayTrade(mainnet_url, times.thirtyDayAgo, "thirty")
  ...
  const ninetyTradeOvm = await fetchDayTrade(optimism_url, times.ninetyDayAgo, "ninety")
  ...
  const ninetyTradeMain = await fetchDayTrade(mainnet_url, times.ninetyDayAgo, "ninety")
